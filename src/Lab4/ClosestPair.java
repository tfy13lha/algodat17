...
public class ClosestPair {
	...
	private void readData(String fileName) {
		try {
			...
			if (line.contains(":")) {
				...
				double x = Double.parseDouble(line.split("\\s+")[1]);
				double y = Double.parseDouble(line.split("\\s+")[2]);
			...
			while (input.hasNextLine()) {
				line = input.nextLine();
				if (line.contains("EOF") || line.matches("^\\s*$")) {
					break;
				}
	...
	private double Recurs(List<Coordinate> xCoords, List<Coordinate> yCoords) {
		...
		double dmin2 = dmin;
		for (int i = 0; i < len; i++) {
			Coordinate ci=yCoords.get(i);
			if (Math.abs(ci.getX() - xstar.getX()) > dmin) {
				continue;
			}
			int checked = 0;
			for (int j = i + 1; (j < len) && (checked < 15); j++) {
				Coordinate cj=yCoords.get(j);
				if(Math.abs(cj.getX() - xstar.getX()) > dmin){
					continue;
				}
				checked++;
				dmin2=Math.min(dmin2, calcDistance(ci,cj));
			}
		}
		return dmin2;
